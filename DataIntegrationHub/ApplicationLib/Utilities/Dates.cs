...
namespace ApplicationLib.Utilities
{
    ...
    public static class Dates
    {
        ...
        public static DateTime? ReadDate(string value, bool convertToNow)
        {
            if (String.IsNullOrEmpty(value))
                return convertToNow ? DateTime.Now : (DateTime?)null;
            try
            {
                var its = value.Split('/');
                return new DateTime(Convert.ToInt32(its[2]), Convert.ToInt32(its[1]), Convert.ToInt32(its[0]));
            }
            catch
            {
                return convertToNow ? DateTime.Now : (DateTime?)null;
        ...
        public static DateTime? ReadTime(string value, bool convertToNow)
        {
            if (String.IsNullOrEmpty(value))
                return convertToNow ? DateTime.Now : (DateTime?)null;
            try
            {
                var its = value.Split(':');
                return new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, Convert.ToInt32(its[0]), Convert.ToInt32(its[1]), Convert.ToInt32(its[2]));
            }
            catch
            {
                return convertToNow ? DateTime.Now : (DateTime?)null;
